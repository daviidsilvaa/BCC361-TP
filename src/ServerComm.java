...
public class ServerComm implements Runnable{
	...
	public void run() {
		...
		while (scanner.hasNextLine()) {
			...
			enlace.solveChecksum(enlace.getFrame()); // Camada de Enlace resolve o checksum
			physical.setFrame(enlace.getFrame()); // Camada Fisica recebe mensagem da Camada de Enlace
			physical.convert4Bto5B();	// camada fisica aplica 4B/5B

			System.out.println("Frame received:\t" + physical.getFrame());

			physical.setFrame(physical.generateError(physical.getFrame(), 2)); // gera error na sequencia de bits do frame recebido pela Camada Física
